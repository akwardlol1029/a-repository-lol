...
spawn(function()
	...
	local char = player.Character or player.CharacterAdded:Wait()
		local hm = WaitForChildOfClass(char, 'Humanoid')
		if not hm then
			task.wait(1)
			hm = WaitForChildOfClass(char, 'Humanoid')
			end
	...
	local c = game:GetService("RunService").Heartbeat:Connect(function()
        ...
        if hm.Health < hm.MaxHealth and hm.Health > hm.MaxHealth/2 then
            bar.BackgroundColor3 = Color3.new(1-(hm.MaxHealth)+0.5,1,0)
        else
            bar.BackgroundColor3 = Color3.fromRGB(27, 252, 107)
        end
        if hm.Health == hm.MaxHealth/2 then
            bar.BackgroundColor3 = Color3.new(1,1,0)
        end
        if hm.Health < hm.MaxHealth/2 then
            bar.BackgroundColor3 = Color3.new(1,(hm.Health/hm.MaxHealth)*2,0)
        end 
	end)
	hm.Died:Wait()
	bar.Size = UDim2.new(0,(hm.Health / hm.MaxHealth * 160),1,0)
...
local function changechatico()
    ...
    elseif ChatIcon.Image == "rbxasset://textures/ui/TopBar/chatOn.png" then
        ChatIcon.Image = "rbxasset://textures/ui/Chat/ChatDown.png"
    end		
end
...
game:GetService("RunService").Heartbeat:Connect(function()
    ...
    if not c00l_mode then
        game.CoreGui:WaitForChild("TopBarApp").LegacyCloseMenu.CloseMenuButton.Image = "rbxasset://textures/ui/Menu/HamburgerDown@2x.png"
    end
